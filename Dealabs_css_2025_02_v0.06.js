...
var nbModifArticles = 0;  // permet d'économiser des ressources en commençant au prochain article non modifié lors du changement de page avec le scroll infini

function modify_css()   
{     
if ( nbModifArticles == 0 ) 
...
if ( !window.location.href.includes("https://www.dealabs.com/search") && !window.location.href.includes("https://www.dealabs.com/groupe") )  
   ...
   for (i=nbModifArticles;i<els_class_listLayout_side.length;i++)  
       {
...
for (i=nbModifArticles;i<els_class_listLayout.length;i++)  
    {
...
for (i=nbModifArticles;i<els_class_threadListCard.length;i++)  
    {
    //els_class_threadListCard[i].style.gridTemplateAreas = '"i b h" "i b  f"';   
    els_class_threadListCard[i].style.gridTemplateAreas =  '"i b f h"';   
    els_class_threadListCard[i].style.gridTemplateColumns="8.0rem 1fr"; // "6.5rem 1fr";   // "8rem 1fr";
...
for (i=nbModifArticles;i<els_class_threadListCard_header.length;i++)  
    {      
...
for (i=nbModifArticles;i<els_class_threadListCard_footer.length;i++)  
    {
...
for (i=nbModifArticles;i<els_articles.length;i++)  
    {
    if ( !els_articles[i].classList.contains("comment") )
       {
       if ( i%2 == 0 )  
          {
          els_articles[i].style.borderBottom = "solid 1px white"; 
          els_articles[i].style.borderLeft   = "solid 1px white"; 
          }
       else           
          {
          els_articles[i].style.borderBottom = "solid 1px #00a1bf"; 
          els_articles[i].style.borderLeft   = "solid 1px #00a1bf"; 
          }
...
for (i=nbModifArticles;i<els_class_userHtml.length;i++)  
    {
    if ( !els_class_userHtml[i].parentElement.classList.contains("comment-body") )
       els_class_userHtml[i].style.display="none";
    }      

nbModifArticles = els_articles.length;    
  
//console.log("fin");
//console.log("fin "+nbModifArticles+" Articles modifiés"); 
} 


// Select the node that will be observed for mutations
const targetNode = document.body;  // document.getElementById("pageContent"); //document.getElementById("toc-target-deals");

// Options for the observer (which mutations to observe)
const config = { attributes: false, childList: true, subtree: true };

// Callback function to execute when mutations are observed
const callback = (mutationList, observer) => { for (const mutation of mutationList) if ( mutation.type === "childList" )  modify_css(); };
